...
(() => {
  ...
  areas.forEach(area => {
    ...
    shape.bindTooltip(`🐕 ${area.name}`,{permanent:true,direction:'center',className:'dog-area-label',opacity:.96});
    shape.addTo(offleashLayer);
  });

  const current=typeof profile==='function' ? profile() : null;
  const hasHome=current && Number.isFinite(Number(current.homeLat)) && Number.isFinite(Number(current.homeLng));
  if (!hasHome) map.setView([52.2182,5.4835],14);

  const offLabel=document.querySelector('[data-layer="offleash"]')?.closest('label');
  if (offLabel) {
    const swatch=offLabel.querySelector('.legend-swatch');
    if (swatch) { swatch.style.background='rgba(24,174,234,.45)'; swatch.style.border='2px solid #005d92'; }
  }
  ...
  if (!document.getElementById('onboardingHome')) {
